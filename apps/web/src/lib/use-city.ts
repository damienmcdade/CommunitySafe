...
export const CITIES: CityInfo[] = [
  ...
  // Boston's CKAN endpoint at data.boston.gov returns zero records from
  // Vercel's serverless runtime even though the same request succeeds from
  // every dev machine I've tried. The failure is independent of response
  // size (we tested 5k → 2k → 500 row limits, all return 0). Most likely
  // an ASN-level filter or strict TLS handshake; needs a different upstream
  // (Socrata mirror or year-segmented CSV) before going live. Keeping the
  // adapter wired so the next change is a one-line status flip back.

...
final class PremiumManager: ObservableObject {
    ...
    /// True when StoreKit never returned the products. The paywall shows a
    /// retry affordance instead of a price it cannot honour.
    ...
    func refresh() async {
        ...
        // Retry with backoff: a single flaky StoreKit round-trip used to leave
        // the paywall with no Product for the whole session, which meant a
        // placeholder price and a buy button that could never complete.
        var products: [Product] = []
        for attempt in 0..<3 {
            products = (try? await Product.products(for: [Self.productID, Self.yearlyProductID])) ?? []
...
struct PaywallView: View {
    ...
    /// Only ever the live StoreKit price. When products haven't loaded the UI
    /// shows a retry rather than inventing a number the purchase can't match.
    var displayPrice: String {
        premium.selectedProduct?.displayPrice ?? "—"
    }
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 28) {
                    ...
                    // Store unreachable: say so and offer a retry, rather than
                    // showing a dead button under an invented price.
                    if premium.productsFailedToLoad {
                        VStack(spacing: 8) {
                            Text("Couldn't reach the App Store to load pricing.")
                                .font(.caption)
                                .foregroundStyle(.secondary)
                            Button("Try again") { Task { await premium.refresh() } }
                    ...
                    VStack(spacing: 12) {
                        ...
                        .disabled(isPurchasing || premium.selectedProduct == nil)
